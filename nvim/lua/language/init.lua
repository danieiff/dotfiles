vim.fn.setenv("CC", "zig")

local ts_parser_ext = (vim.uv.os_uname().sysname:find("Windows") and ".dll" or ".so")
local builtin_ts_parsers = { "lua", "vim", "vimdoc", "markdown", "markdown_inline", "c" }
for _, lang in ipairs(builtin_ts_parsers) do
  vim.treesitter.language.add(
    lang,
    { path = vim.fn.getenv("VIM") .. "/../../lib/nvim/parser/" .. lang .. ts_parser_ext }
  )
end
...
-- incremental selection: v an in [n ]n
...
local lsp_config_names = vim.iter(installed_packs):fold({}, function(acc, pack)
  if pack.spec.neovim and not vim.iter(disabled_lsps):find(pack.spec.neovim.lspconfig) then
    table.insert(acc, pack.spec.neovim.lspconfig)
  end
  return acc
end)
...
K("<leadler>tf", function()
  neotest.run.run(vim.fn.expand("%"))
end)
K("<leader>td", function()
...
K("<Leader>dI", function()
  dap.set_breakpoint(vim.fn.input("Breakpoint condition: "))
end)
K("<Leader>dp", function()
...
K("<Leader>dww", function()
  dap.toggle()
end)
K("<Leader>dw[", function()
  dap.toggle(1)
end)
K("<Leader>dw]", function()
  dap.toggle(2)
...
dap.listeners.before["event_progressStart"]["progress-notifications"] = function(session, body)
  vim.notify(
    (body.percentage and body.percentage or "") .. "%\t" .. body.message,
    vim.log.levels.INFO,
...
require('dap.ext.vscode').json_decode = function(str)
  str = str:gsub(",(%s*[}%]])", '%1') -- remove trailing comma
    ...
    function(res)
      ...
      for ls in founds do
        ...
        vim.system({
          ...
          "-fsSL",
          (
            "https://raw.githubusercontent.com/neovim/nvim-lspconfig/master/lua/lspconfig/server_configurations/"
            .. fname
          ),
        }, {}, function(curl_lsconfig_res)
          if curl_lsconfig_res.code ~= 0 then
            return vim.notify(curl_lsconfig_res.stderr)
          end
    ...
    K("cv", function()
      vim.lsp.buf.rename(nil, {
        filter = function(_client)
          ...
          return not vim.tbl_contains(deprioritised, _client.name)
              or #vim.lsp.get_clients({ method = "textDocument/rename" }) < 1
        end,
      })
    end)
